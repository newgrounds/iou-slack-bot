...
app.post('/', function (req, res, next) {
  if (!/^$/i.test(req.body.text)) return next()
...
// /iou history
app.post('/', function (req, res, next) {
  if (!/^history$/i.test(req.body.text)) return next()
...
// /iou @user $20
app.post('/', function (req, res, next) {
  var cmdMatch = req.body.command.match(/^\/iou/i)
  if (!cmdMatch) return next()
  
  var match = req.body.text.match(/^<@([0-9A-Z]+)\|(?:[0-9A-Z]+)>\s+[\$€]?([0-9]+)(?:\s+(.+))?$/i)
...
// /ask @user $20
app.post('/', function (req, res, next) {
  var cmdMatch = req.body.command.match(/^\/ask/i)
  if (!cmdMatch) return next()
...
// /iou @user
app.post('/', function (req, res, next) {
  var match = req.body.text.match(/^<@([0-9A-Z]+)\|(?:[0-9A-Z]+)>/i)
...
// didn't understand the request or /iou help
app.post('/', function (req, res, next) {
  var text = []

  if (!/^help$/i.test(req.body.text)) {
...
function toHistoryItem (debitor, note) {
  ...
  if (debitor === note.debitor) {
    text += ` you owe <@${note.creditor}> ${note.amount}.`
  } else {
    text += ` <@${note.debitor}> owes you ${note.amount}.`
  }
